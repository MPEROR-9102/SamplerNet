...
def train(interpolator, train_loader, test_loader, cross_loader, loss_fn, optimizer, scheduler,\
          epochs, sample_type="random", save_path=""):
    ...
    for epoch in range(epochs):
        ...
        if val_total_loss < best_val_loss:
            ...
            print(f"Boston(Interpolation) Test Loss: {validate(interpolator, cross_loader, sample_func, loss_fn)}")
            # model_dict = {
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', '--sample_rate', type=float)
    ...
    pmnet_train_size = int(dataset_size * 0.9)
    ...
    pmnet_train =  DataLoader(pmnet_train_dataset, batch_size=16, shuffle=True, generator=torch.Generator(device="cpu"))
    pmnet_test = DataLoader(pmnet_test_dataset, batch_size=16, shuffle=False, generator=torch.Generator(device="cpu"))

    data_root1 = "Boston/"
    num_of_maps1 = 3143
    ddf1 = pd.DataFrame(np.arange(1, num_of_maps1))
    ddf1.to_csv('boston.csv', index=False)
    boston_dataset = PMnet_boston(csv_file = "boston.csv", dir_dataset=data_root1)
    boston_loader = DataLoader(boston_dataset, batch_size=16, shuffle=False, generator=torch.Generator(device="cpu"))
    ...
    key = f"{round(target_sample_rate, 1)}"
    ...
    best_test_loss = train(interpolator, pmnet_train, pmnet_test, boston_loader, loss_fn,\
                              optimizer, scheduler, epochs=30, sample_type='random')
