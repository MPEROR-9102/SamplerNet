...
def interpolator_train(interpolator, train_loader, test_loader, loss_fn,\
    ...
    for epoch in range(epochs):
        ...
        val_total_loss = interpolator_validate(interpolator, test_loader, sample_func, loss_fn)
...
def train(model, sample_rate, interpolator, train_loaders, test_loader, optimizer, scheduler, epochs):
    best_val_loss = float("inf")
        ...
        with tqdm(total=total_batches, desc=f"Epoch {epoch+1}/{epochs} - Training", unit="batch") as pbar:
            for _type, train_loader in train_loaders:
                for inputs, targets in train_loader:
                    ...
                    if _type == 'partial':
                        with torch.no_grad():
                            targets = rand_sampling(targets, (inputs[:,0] == 0).int().unsqueeze(1), sample_rate)
                            partial_samples_count += (targets > 0).int().sum()
                            targets = interpolator(torch.cat([inputs[:,0].unsqueeze(1), targets], dim=1))
...
def compensated_pmnet_train(model, sample_rate, partials, train_loaders, test_loader, optimizer, scheduler, epochs):
    best_val_loss = float("inf")
...
def interpolator_validate(interpolator, test_loader, sample_func, loss_fn):
    interpolator.eval()
    total_loss = 0
    with torch.no_grad():
...
if __name__ == "__main__":
    ...
    key = f"Interpolator-TH{round(train_hyper, 1)}-SR{round(target_sample_rate, 1)}"   
    ...
    best_test_loss = interpolator_train(interpolator, train_full, train_partial, loss_fn, optimizer, scheduler, 30)
    best_test_losses[key] = best_test_loss
    
    
    #1 Fully-Sampled + Partially Sampled   
    key = f"PMNet-TH{round(train_hyper, 1)}-SR{round(target_sample_rate, 1)}"    
    ...
    print(key, "Training Started...")
    best_test_loss, partial_samples_count = train(model, target_sample_rate, interpolator, pmnet_train,\
